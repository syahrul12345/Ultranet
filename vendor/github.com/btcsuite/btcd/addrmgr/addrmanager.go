...
const (
	...
	dumpAddressInterval = time.Minute
...
// updateAddressRelaxed updates an address without checking whether or not it's
// routable first. Useful for testing situations.
func (a *AddrManager) updateAddressRelaxed(netAddr, srcAddr *wire.NetAddress) {
	...
	if ka != nil {
		...
		// likelihood is 2N after we have 100 addresses.
		if ka.refs > 100 {
	...
	if len(a.addrNew[bucket]) > newBucketSize {
		glog.Tracef("new bucket is full, expiring old")
		a.expireNew(bucket)
	}
	...
	glog.Tracef("Added new address %s for a total of %d addresses", addr,
		a.nTried+a.nNew)
}
...
func (a *AddrManager) updateAddress(netAddr, srcAddr *wire.NetAddress) {
	...
	if !IsRoutable(netAddr) {
		glog.Debugf("AddressManager.updateAddress: Filtering out non-routable address %v:%v", netAddr.IP, netAddr.Port)
		return
	}

	a.updateAddressRelaxed(netAddr, srcAddr)
}
...
func (a *AddrManager) expireNew(bucket int) {
	...
	for k, v := range a.addrNew[bucket] {
		if v.isBad() {
			glog.Tracef("expiring bad address %v", k)
	...
	if oldest != nil {
		...
		glog.Tracef("expiring oldest address %v", key)
...
out:
	...
	a.savePeers()
	a.wg.Done()
	glog.Trace("Address handler done")
}
...
func (a *AddrManager) savePeers() {
	...
	if err != nil {
		glog.Errorf("Error opening file %s: %v", a.peersFile, err)
		return
	}
	enc := json.NewEncoder(w)
	defer w.Close()
	if err := enc.Encode(&sam); err != nil {
		glog.Errorf("Failed to encode file %s: %v", a.peersFile, err)
		return
	}
	log.Tracef("Saved peers to file: %s", a.peersFile)
}
...
func (a *AddrManager) loadPeers() {
	...
	if err != nil {
		glog.Errorf("Failed to parse file %s: %v", a.peersFile, err)
		// if it is invalid we nuke the old one unconditionally.
		err = os.Remove(a.peersFile)
		if err != nil {
			glog.Warningf("Failed to remove corrupt peers file %s: %v",
				a.peersFile, err)
		}
		a.reset()
		return
	}
	glog.Infof("Loaded %d addresses from file '%s'", a.numAddresses(), a.peersFile)
}
...
func (a *AddrManager) Start() {
	...
	glog.Info("Starting address manager")
...
func (a *AddrManager) Stop() error {
	if atomic.AddInt32(&a.shutdown, 1) != 1 {
		glog.Warningf("Address manager is already in the process of " +
			"shutting down")
		return nil
	}

	glog.Infof("Address manager shutting down")
	close(a.quit)
	a.wg.Wait()
	glog.Infof("Address manager successfully shut down")
	return nil
}

// AddAddressesRelaxed adds addresses without checking if they're routable first. This
// is useful in testing situations where we want the address manager to contain localhost
// addresses e.g. to run multiple nodes on the same machine and have them be abel to
// connect to each other.
func (a *AddrManager) AddAddressesRelaxed(addrs []*wire.NetAddress, srcAddr *wire.NetAddress) {
	a.mtx.Lock()
	defer a.mtx.Unlock()

	for _, na := range addrs {
		a.updateAddressRelaxed(na, srcAddr)
	}
}
...
// GetAllAddrs simply returns all the addresses that we have.
func (a *AddrManager) GetAllAddrs() []*wire.NetAddress {
	return a.getAddresses()
}

// AddressCacheNumAddrs returns the number of addrs desired from the
// addrmgr.
func (a *AddrManager) AddressCacheNumAddrs(numAddresses int) []*wire.NetAddress {
...
func (a *AddrManager) AddressCache() []*wire.NetAddress {
	...
	return a.AddressCacheNumAddrs(numAddresses)
}
...
func HostToNetAddress(host string, port uint16, services wire.ServiceFlag) (*wire.NetAddress, error) {
...
func (a *AddrManager) GetAddress() *KnownAddress {
	return a.GetAddressWithExclusions(make(map[string]bool))
}
func (a *AddrManager) GetAddressWithExclusions(excludedAddrs map[string]bool) *KnownAddress {
	...
	//
	// Additionally only look for a tried address if we're sure that there
	// exists a tried address that isn't excluded.
	var lastKnownAddressFound *KnownAddress
	maxOverallAttempts := 100
	for overallAttempts := 0; overallAttempts < maxOverallAttempts; overallAttempts++ {
		maxInnerAttempts := 1000
		if a.nTried > 0 && (a.nNew == 0 || a.rand.Intn(2) == 0) {
			// Tried entry.
			large := 1 << 30
			factor := 1.0
			for numAttempts := 0; numAttempts < maxInnerAttempts; numAttempts++ {
				...
				// Don't consider addresses we've excluded. Usually this is used
				// to prevent us from returning addresses our node is already
				// connected to.
				if _, ok := excludedAddrs[NetAddressKey(ka.na)]; ok {
					continue
				}

				lastKnownAddressFound = ka
				randval := a.rand.Intn(large)
				if float64(randval) < (factor * ka.chance() * float64(large)) {
					glog.Tracef("Selected %v from tried bucket",
						NetAddressKey(ka.na))
					return ka
				}
				factor *= 1.2
			...
			for numAttempts := 0; numAttempts < maxInnerAttempts; numAttempts++ {
				...
				randval := a.rand.Intn(large)
				if float64(randval) < (factor * ka.chance() * float64(large)) {
					glog.Tracef("Selected %v from new bucket",
						NetAddressKey(ka.na))
					return ka
				}
				factor *= 1.2
...
func (a *AddrManager) Good(addr *wire.NetAddress) {
	...
	glog.Tracef("Replacing %s with %s in tried", rmkey, addrKey)
...
func (a *AddrManager) AddLocalAddress(na *wire.NetAddress, priority AddressPriority) error {
	...
	glog.Infof("Adding local address: %s:%d\n", na.IP.String(), na.Port)
...
func (a *AddrManager) GetBestLocalAddress(remoteAddr *wire.NetAddress) *wire.NetAddress {
	...
	if bestAddress != nil {
		glog.Tracef("Suggesting address %s:%d for %s:%d", bestAddress.IP,
			bestAddress.Port, remoteAddr.IP, remoteAddr.Port)
	} else {
		glog.Tracef("No worthy address for %s:%d", remoteAddr.IP,
			remoteAddr.Port)
