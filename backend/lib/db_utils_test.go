...
func _GetTestBlockNode() *BlockNode {
	...
	// Header (make a copy)
	bs.Header = NewMessage(MsgTypeHeader).(*MsgUltranetHeader)
...
func _GetTestBadgerDb() (_db *badger.DB, _dir string) {
...
func TestInitDbWithGenesisBlock(t *testing.T) {
	...
	err := InitDbWithUltranetGenesisBlock(&UltranetTestnetParams, db)
	require.NoError(err)
	...
	genesisHash := *NewBlockHash(UltranetTestnetParams.GenesisBlockHashHex)
