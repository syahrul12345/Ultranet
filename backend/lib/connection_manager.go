...
func NewConnectionManager(
	_params *UltranetParams, _addrMgr *addrmgr.AddrManager, _listeners []net.Listener,
	_connectIps []*wire.NetAddress, _timeSource chainlib.MedianTimeSource,
	...
	return &ConnectionManager{
		...
		// self connections.
		sentNonces: peer.NewMruNonceMap(1000),
...
func (cmgr *ConnectionManager) getRandomAddr() *wire.NetAddress {
	for tries := 0; tries < 100; tries++ {
		...
		addr := cmgr.addrMgr.GetAddressWithExclusions(cmgr.connectedOutboundAddrs)
...
// IPToNetAddr ...
func IPToNetAddr(ipStr string, params *UltranetParams) (*wire.NetAddress, error) {
...
func (cmgr *ConnectionManager) _connectPeer(conn net.Conn, persistentAddr *wire.NetAddress) {
	...
	for {
		...
		if conn == nil {
			...
			if isPersistent {
				glog.Errorf("_connectPeer: Got a nil connection for a persistent peer. This should never happen: (%s)", persistentAddr.IP.String())
			}
		...
		na, err := IPToNetAddr(conn.RemoteAddr().String(), cmgr.params)
		if err != nil {
			glog.Errorf("_connectPeer: Problem calling ipToNetAddr for addr: (%s) err: (%v)", conn.RemoteAddr().String(), err)
		...
		peer := NewPeer(conn, isOutbound, na, isPersistent,
			cmgr.stallTimeoutSeconds, cmgr.minFeeRateNanosPerKB, cmgr, cmgr.srv)

		if err := peer.negotiateVersion(); err != nil {
			glog.Debugf("_connectPeer: Problem negotiating version with peer with addr: (%s) err: (%v)", conn.RemoteAddr().String(), err)
...
func (cmgr *ConnectionManager) _initiateOutboundConnections() {
	if len(cmgr.connectIps) > 0 {
		...
		for _, ipNetAddr := range cmgr.connectIps {
			go func(na *wire.NetAddress) {
				cmgr._connectPeer(nil, na)
			}(ipNetAddr)
		}
		return
	}
...
func (cmgr *ConnectionManager) _isFromRedundantInboundIPAddress(addrToCheck net.Addr) bool {
	...
	netAddr, err := IPToNetAddr(addrToCheck.String(), cmgr.params)
	if err != nil {
		...
		go func(ll net.Listener) {
			for {
				...
				if atomic.LoadInt32(&cmgr.shutdown) != 0 {
					glog.Trace("_handleInboundConnections: Ignoring connection due to shutdown")
					return
				}
				if err != nil {
					glog.Debugf("_handleInboundConnections: Can't accept connection: %v", err)
					continue
				}
				...
				if numInboundPeers > cmgr.maxInboundPeers {
					...
					glog.Debugf("Rejecting INBOUND peer (%s) due to max inbound peers (%d) hit.",
				...
				if cmgr.limitOneInboundConnectionPerIP &&
					...
					glog.Debugf("Rejecting INBOUND peer (%s) due to already having an "+
		...
		case pp := <-cmgr.newPeerChan:
			{
				...
				if pp.isOutbound &&
					...
					glog.Debugf("Rejecting OUTBOUND NON-PERSISTENT peer (%v) with "+
				...
				if !pp.isOutbound && numInboundPeers > cmgr.maxInboundPeers {
					...
					glog.Debugf("Rejecting INBOUND peer (%v) due to max inbound peers (%d) hit.",
