...
type mruInventoryMap struct {
	invMtx  deadlock.Mutex
	invMap  map[InvVect]*list.Element // nearly O(1) lookups
...
//
// This function is safe for concurrent access.
func (m *mruInventoryMap) Exists(iv *InvVect) bool {
	m.invMtx.Lock()
...
//
// This function is safe for concurrent access.
func (m *mruInventoryMap) Add(iv *InvVect) {
	m.invMtx.Lock()
	defer m.invMtx.Unlock()
...
// deleteInvVect deletes the inventory vector without acquiring
// any locks.
func (m *mruInventoryMap) deleteInvVect(iv *InvVect) {
	if node, exists := m.invMap[*iv]; exists {
...
func (m *mruInventoryMap) Delete(iv *InvVect) {
	m.invMtx.Lock()
	defer m.invMtx.Unlock()
...
func (m *mruInventoryMap) DeleteListingHash(bh *BlockHash) {
	m.invMtx.Lock()
	defer m.invMtx.Unlock()
	...
	m.deleteInvVect(&invVect)
}

func (m *mruInventoryMap) DeleteBlockHash(bh *BlockHash) {
	m.invMtx.Lock()
	defer m.invMtx.Unlock()
...
func (m *mruInventoryMap) DeleteTxHash(txHash *BlockHash) {
	m.invMtx.Lock()
	defer m.invMtx.Unlock()
...
func (m *mruInventoryMap) Reset() {
	m.invMap = make(map[InvVect]*list.Element)
	m.invList = list.New()
}
...
func newMruInventoryMap(limit uint) *mruInventoryMap {
	m := mruInventoryMap{
		invMap:  make(map[InvVect]*list.Element),
		invList: list.New(),
		limit:   limit,
	}
	return &m
